...
// redraw moves the cursor back. A nil renderer keeps the raw stream 鈥?correct
...
func (s *TextSink) Emit(e event.Event) {
	...
	case event.Reasoning:
		if !s.wroteReasoningHeader {
			fmt.Fprintln(s.out, dimText("  鈻?thinking"))
	...
	case event.ToolDispatch:
		// The early (Partial) dispatch carries no args 鈥?the full one prints the
	...
	case event.ToolResult:
		...
		// call surfaces the same "鈯?name <reason>" line the agent used to print.
		if e.Tool.Err != "" {
			fmt.Fprintf(s.out, "  鈯?%s %s\n", e.Tool.Name, e.Tool.Err)
			s.wroteAnything = true
		}
	...
	case event.CompactionStarted:
		fmt.Fprintln(s.out, dimText("  鈰?compacting conversation鈥?))
		s.wroteAnything = true

	case event.CompactionDone:
		c := e.Compaction
		if c.Summary == "" {
			break // aborted pass 鈥?the caller's Notice already explained why
		}
		fmt.Fprintln(s.out, dimText(fmt.Sprintf("  鈰?compacted %d messages (%s)", c.Messages, c.Trigger)))
...
// FormatUsageLine renders the per-turn token/cache summary 鈥?the key signal for
// the cache-first design 鈥?as a single line (no trailing newline), or "" when
...
func FormatUsageLine(u *provider.Usage, p *provider.Pricing, d *event.CacheDiagnostics) string {
	...
	cost := ""
	if p != nil {
		cost = fmt.Sprintf(" 路 %s%.4f", p.Symbol(), p.Cost(u))
	}
	churn := ""
	if d != nil && d.PrefixChanged {
		...
		churn = fmt.Sprintf(" 路 cache prefix changed: %s", reasons)
	}
	return fmt.Sprintf("  路 %d tok 路 in %d%s 路 out %d%s%s%s",
