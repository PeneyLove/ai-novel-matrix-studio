...
const systemPrompt = "you are novel-agent, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("novel-agent_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set novel-agent_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("novel-agent_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("novel-agent_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("novel-agent_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
