...
// it to the registry â€?the same pattern as the task tool â€?rather than
...
func (rememberTool) Description() string {
	return "Save a durable fact to project memory so it survives across sessions. " +
		...
		"Before saving, check the loaded memory index for an entry that already covers this â€?reuse that name to update it rather than create a near-duplicate, and use `forget` to drop one that is now wrong. " +
...
func (rememberTool) Schema() json.RawMessage {
	return json.RawMessage(`{
		"type": "object",
		"properties": {
			"name": {"type": "string", "description": "Short kebab-case slug identifying the fact, e.g. \"prefers-tabs\". Reusing a name overwrites that memory â€?do that to update an existing fact. Omit to derive one from the description."},
			...
			"description": {"type": "string", "description": "One-line hook shown in the index â€?the phrase a future session reads to decide whether to open this memory. Make it specific."},
