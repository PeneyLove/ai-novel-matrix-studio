// Package memory implements novel-agent's persistent memory. It mirrors Claude
// Code's two-layer model while honoring novel-agent's cache-first architecture:
//
//   - Hierarchical doc memory: novel-agent.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/novel-agent/novel-agent.md
	ScopeAncestor Scope = "ancestor" // a novel-agent.md above the project root
	ScopeProject  Scope = "project"  // ./novel-agent.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./novel-agent.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// novel-agent.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"novel-agent.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"novel-agent.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
