...
const missingDescPlaceholder = `(no description â€?frontmatter is missing a "description:" line; tell the user to add one)`
...
const indexHeader = "# Skills â€?playbooks you can invoke\n\n" +
	"One-liner index. Before non-trivial work, scan it: if an untagged (inline) skill is even plausibly relevant to the task, invoke it before continuing instead of pre-judging â€?loading one imperfect inline skill is cheap. Skills tagged `[ðŸ§¬ subagent]` are the heavy path; reach for them only when the task genuinely needs context-heavy work, not on weak relevance. Each entry is a built-in or a user-authored playbook. Call `run_skill({ name: \"<skill-name>\", arguments: \"<task>\" })` â€?`name` is JUST the identifier (e.g. `\"explore\"`), NOT the `[ðŸ§¬ subagent]` tag that follows it. Prefer the dedicated top-level tool when one exists for a built-in subagent skill. Entries tagged `[ðŸ§¬ subagent]` spawn an isolated subagent â€?its tool calls and reasoning never enter your context, only its final answer does; use them for context-heavy work (deep exploration, multi-step research) where you only need the conclusion. Untagged skills are inlined: the body becomes a tool result you read and act on directly. The user can also invoke a skill via `/<name>`."
...
func ApplyIndex(basePrompt string, skills []Skill) string {
	...
	if r := []rune(joined); len(r) > IndexMaxChars {
		joined = string(r[:IndexMaxChars]) + fmt.Sprintf("\nâ€?(truncated %d chars)", len(r)-IndexMaxChars)
	}
	return basePrompt + "\n\n" + indexHeader + "\n\n```\n" + joined + "\n```"
}

// indexLine renders one skill as "- name [tag] â€?description", clipped to a
