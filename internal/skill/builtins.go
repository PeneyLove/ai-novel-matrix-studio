...
const negativeClaimRule = `When you claim something does NOT exist (no caller, no usage, not implemented), say which searches you ran to reach that conclusion â€?a negative claim is only as trustworthy as the search behind it.`
...
- For "find all places that call / reference / use X" questions, use ` + "`grep`" + ` (content search) â€?NOT ` + "`glob`" + ` (which only matches file names). Using the wrong one gives empty results and wastes your budget.
...
- Don't read every file â€?be selective. Breadth on the first pass, depth only where the question demands it.
...
How to operate:
- Combine code reading (read_file, grep, glob) with web_fetch as appropriate. (There is no dedicated web-search tool â€?fetch the canonical doc/spec URL directly when you know it.)
...
- Distinguish "I verified this in code" from "I read this on a docs page" â€?the parent trusts the former more.
...
const builtinReviewBody = `You are running as a code-review subagent. Inspect the changes the user is about to ship â€?usually the current git branch vs its upstream â€?and produce a focused review the parent can hand back.
...
- Read touched files (read_file) when the diff alone lacks context â€?signatures, surrounding invariants, callers.
...
1. Correctness bugs â€?off-by-one, nil handling, races, wrong operator, unhandled edge cases.
2. Security â€?injection (SQL, shell, path traversal), secrets, missing authz, unsafe deserialization.
3. Behavior changes the diff hides â€?renames missing callers, removed load-bearing branches, error-handling that now swallows what used to surface.
4. Tests â€?does the change have tests for the new behavior? Are existing tests still meaningful?
5. Style + consistency â€?only flag deviations that matter; don't pile on cosmetic nits if the substance is clean.
...
const builtinSecurityReviewBody = `You are running as a security-review subagent. Inspect the changes the user is about to ship â€?usually the current git branch vs its upstream â€?through a security lens specifically, and report exploitable issues.

How to operate:
- Default scope: the current branch's diff vs the default branch. Honor a named range or directory if given.
- Discover scope first: ` + "`bash git status`" + `, ` + "`git diff --stat`" + `, ` + "`git diff <base>...HEAD`" + `. Read touched files (read_file) when the diff lacks security context â€?auth checks, input validation, the handler that calls the changed code.
...
const builtinTestBody = `This skill is INLINED â€?you run in the parent loop. The user asked you to run the tests and fix failures. Run the project's test suite, diagnose any failure, propose and apply fixes, then re-run. Repeat until green or you hit a wall worth escalating.

How to operate:
1. Detect the test command. Look at the project: go.mod â†?` + "`go test ./...`" + `; package.json scripts.test â†?` + "`npm test`" + ` (or pnpm/yarn); pyproject.toml/requirements.txt â†?` + "`pytest`" + `; Cargo.toml â†?` + "`cargo test`" + `. If you can't tell, ASK â€?don't guess.
...
4. Fix each distinct failure:
   - Production bug (test caught a real defect) â†?fix the production code.
   - Test bug (test is wrong, code is right) â†?fix the test, and say so explicitly.
   - Environmental (missing dep, wrong toolchain, missing fixture) â†?say so and stop; don't install packages or change config without checking.
5. Apply the edit and re-run. Iterate.
6. Stop conditions: all green â†?report what changed; same test still failing after 2 attempts on the same line â†?STOP and explain; 3+ unrelated failures â†?fix one at a time, smallest first.
...
Lead each turn with a one-line status (e.g. "â–?running go test ./â€?", "â–?2 failures in foo_test.go â€?first is â€?) so the user always knows where you are.`

const builtinInitBody = `This skill is INLINED â€?you run in the parent loop. The user invoked /init: bootstrap (or refresh) this project's AGENTS.md â€?the durable memory file folded into every future session. Analyze the codebase, then write a concise, high-signal AGENTS.md.

How to operate:
1. Check for an existing memory doc first: list the project root and look for AGENTS.md / REASONIX.md / CLAUDE.md. If one exists, read it and IMPROVE it in place (fix stale facts, fill gaps) â€?write back to that same filename, don't clobber it wholesale or create a second file.
2. Explore enough to be accurate, not exhaustive:
   - Project shape: ls / directory listing, the manifest (go.mod, package.json, pyproject.toml, Cargo.toml, â€?, the README.
   - Build / test / run commands: derive them from the manifest + scripts and verify the exact names â€?don't guess.
   - Architecture: the main packages/modules and how they fit; the entry point(s).
   - Conventions: formatting, naming, error handling, testing patterns â€?infer from real code (read a few representative files), not assumptions.
3. Write AGENTS.md with write_file (default filename AGENTS.md, unless an existing doc uses another name), each section terse:
   ...
   - ## Project â€?what it is, the stack, where the entry point lives.
   - ## Commands â€?the exact build / test / run / lint commands.
   - ## Architecture â€?the 3-7 load-bearing modules and their roles.
   - ## Conventions â€?only rules an agent must follow (style, patterns, do/don't).
   - ## Notes â€?leave an empty stub for later quick-adds.
4. Keep it tight â€?it loads into every session's prompt, so every line costs context. Prefer specifics (file paths, command names) over prose. Never include secrets.

Rules:
- Verify commands and paths against the actual files before writing them â€?a wrong build command is worse than none.
...
func builtinSkills() []Skill {
	...
	return []Skill{
		{
			Name:        "init",
			Description: "Bootstrap or refresh this project's AGENTS.md â€?analyze the codebase (structure, build/test commands, architecture, conventions) and write a concise memory file loaded into every future session. Inlined â€?runs in the main loop so you see and approve the write.",
		...
		{
			Name:         "explore",
			Description:  "Explore the codebase in an isolated subagent â€?wide-net read-only investigation that returns one distilled answer. Best for: 'find all places that...', 'how does X work across the project', 'survey the code for Y'.",
		...
		{
			Name:         "review",
			Description:  "Review the pending changes (current branch diff by default) in an isolated subagent â€?flags correctness, security, missing tests, hidden behavior changes; reports a verdict + per-issue file:line. Read-only.",
		...
		},
		{
			...
			Description:  "Security-focused review of the current branch diff in an isolated subagent â€?flags injection/authz/secrets/deserialization/path-traversal/crypto issues, severity-tagged. Read-only.",
		...
		{
			Name:        "test",
			Description: "Run the project's test suite, diagnose failures, propose+apply fixes, re-run until green (or stop after 2 attempts on the same failure). Inlined â€?runs in the parent loop. Detects go/npm/pnpm/yarn/pytest/cargo.",
