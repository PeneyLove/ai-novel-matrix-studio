...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/codegraph"
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/instruction"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/jobs"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/lsp"
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/sandbox"
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	// Initialize RAG tool configuration from [rag] config section.
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "novel-agent.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "novel-agent.toml" // match Config.Save() fallback
	}
	return path
}
