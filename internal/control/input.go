...
const PlanModeMarker = "[Plan mode â€?read-only. Explore the codebase first (read_file, ls, grep, glob, web_fetch, task are available; writers are refused by the harness), then present a LAYERED plan as your reply and stop â€?do not write files, edit, or run side-effecting bash. Structure the plan as a two-level markdown list so it becomes a layered task list: each PHASE is a top-level numbered list item (a coherent milestone, e.g. \"1. Add the config loader\"), and each phase's concrete, verifiable sub-steps are bullets indented beneath it (e.g. \"   - parse the TOML into Config\"). Use plain numbered list items for phases â€?do NOT write phases as markdown headings (##, ###) â€?so both levels parse. Keep phases few (about 2-6). The user will be asked to approve before any changes are made.]"
...
// after "#" is intentional: "#7", "#issue", and "#æ ‡é¢˜" are ordinary user
...
// CustomCommand resolves a "/name argsâ€? line against the loaded custom slash
// commands, returning the rendered prompt to send (found=false when no command
// matches). It does not apply the plan-mode marker â€?call Compose for that.
...
// RunSkill resolves a "/<name> argsâ€? line against the loaded skills, returning
// the skill's rendered body to send as a turn (found=false when no skill
// matches). Invoking a skill by slash always inlines its body â€?the model reads
...
// MCPPrompt resolves a "/mcp__server__prompt argsâ€? line: it maps the positional
