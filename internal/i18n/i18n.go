...
//
// Adding a field requires updating every messages_*.go file 鈥?drift is caught
...
//
// Scope (v1): CLI surface only 鈥?welcome, init wizard, chat REPL banner, usage,
...
// fmt.Sprintf. Catalogue values do not include trailing newlines 鈥?call sites
...
type Messages struct {
	...
	WelcomeTitleFmt string // first-run box title 鈥?%s = product name (styled)
	NoConfigYet     string // first-run cue under the welcome box
	StartingChatFmt string // "Starting %s鈥? before dropping into chat
	...
	ConfigErrorFmt  string // "%s 鈥?error: %v" 鈥?config path + parse error
	NoKey           string // status dot 鈥?no API key set
	Ready           string // status dot 鈥?provider ready
	GetStarted      string // section title above numbered steps
	StepScaffold    string // step 1 desc 鈥?reasonix setup
	StepSetKey      string // step 2 command label

	// `reasonix init` 鈥?points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc 鈥?env var hint
	...
	ChatThinking           string // live reasoning marker label, e.g. "thinking鈥?
	ChatThoughtForFmt      string // collapsed reasoning summary, "%d" = elapsed s
	ChatStatusThinkingFmt  string // "%s thinking鈥?(%ds 路 <cancel hint>)" 鈥?%s = spinner, %d = elapsed s
	ChatToolWorkingFmt     string // "%s working 路 %ds" under a running tool 鈥?%s = spinner, %d = elapsed s
	ChatStatusRetryingFmt  string // "%s retrying (%d/%d)鈥? 鈥?%s = spinner, %d/%d = attempt/max
	...
	ToolApprovalPromptFmt  string // approval banner 鈥?tool, subject suffix, and source/intent detail
	ToolApprovalSourceFmt  string // "Source: %s" / "鏉ユ簮: %s"
	...
	DiffFoldedFmt          string // "鈥?+%d more lines" footer when a writer diff is folded
	...
	CompactionTitle   string // card header before "路 N messages 路 <trigger>"
	...
	SkillPickerMatchingFmt       string // "%d matching 路 %d total" when searching
	...
	SkillPickerMoreAboveFmt      string // "鈫?%d more above"
	SkillPickerMoreBelowFmt      string // "鈫?%d more below"
	...
	WroteFileFmt          string // "Wrote %s" 鈥?used for reasonix.toml and .env both
	SetupComplete         string // success line at end of init
	SetupCancelled        string // shown when the user aborts the wizard
	TryHintFmt            string // "Try: %s" 鈥?%s = command to try (styled)
	...
	CustomFetchEmpty           string // "/models returned an empty list 鈥?falling back to manual entry"
	AnthropicFetchEmpty        string // "/models returned an empty list 鈥?Anthropic-compatible providers usually don't expose one, falling back to manual entry"
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from reasonix.toml (pointing at %s) 鈥?please remove it"
	...
	ErrorPrefix               string // "error:" 鈥?prefix for fatal-error output
	ReconfigureOnUnknownModel string // shown when the configured model no longer resolves and setup is re-run
	WriteConfigErr            string // "write config:" 鈥?prefix for write failure
	WriteEnvErr               string // "write .env:" 鈥?prefix for env-write failure

	// provider HTTP error explanations 鈥?actionable, reason + fix per status code
	...
	// selection menus
	SelectOneHint  string // "(鈫?鈫?路 Enter 路 q to cancel)"
	SelectManyHint string // "(鈫?鈫?路 Space 路 Enter 路 q)"
...
func normalize(s string) string {
	...
	if strings.HasPrefix(s, "zh") || strings.Contains(s, "chinese") || strings.Contains(s, "涓�枃") {
		return "zh"
	}
