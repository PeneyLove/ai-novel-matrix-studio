...
func TestSandboxEnforcesWrites(t *testing.T) {
	...
	workRoot, err := os.MkdirTemp(home, ".novel-agent-sbtest-work-*")
	if err != nil {
		t.Skipf("cannot create work dir under home: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(workRoot) })
	outside, err := os.MkdirTemp(home, ".novel-agent-sbtest-out-*")
	if err != nil {
...
func TestGoBuildUnderSandbox(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".novel-agent-sbtest-go-*")
	if err != nil {
		t.Skipf("cannot create work dir under home: %v", err)
	}
