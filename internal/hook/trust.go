...
// Trust gates project hooks. A project's .novel-agent/settings.json can run
...
// root. The trust flag lives in user-global state (~/.novel-agent/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.novel-agent/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.novel-agent.
...
// TrustPath is ~/.novel-agent/trust.json (homeDir overrides ~).
