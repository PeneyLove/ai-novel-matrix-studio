// Package cli implements novel-agent's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `novel-agent init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
	...
	case "version", "--version", "-v":
		fmt.Println("novel-agent", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	fmt.Printf("novel-agent serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the novel-agent-owned global credentials file so they never land
...
// project-local novel-agent.toml only when the user config dir can't be resolved.
...
// defaultEnvTarget is the novel-agent-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `novel-agent setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./novel-agent.toml
...
// setupConfig runs the configuration wizard (the `novel-agent setup` command),
// writing config.toml to the user-global dir (or ./novel-agent.toml under --local)
// and API keys to the novel-agent-owned global .env — never a project's own .env.
...
// initHint handles `novel-agent init`. Unlike a config scaffold, project memory is
...
// (and to `novel-agent setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the novel-agent-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("novel-agent")),
		"",
...
// that older versions of the wizard wrote into novel-agent.toml. They collide
...
// can warn the user to clean up novel-agent.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `novel-agent setup`.
// The host-based slug also gives users a meaningful name to grep for in
// novel-agent.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing novel-agent.toml deal with SKU names
...
// novel-agent.toml that defines only [[providers]] for deepseek otherwise hides
...
// novel-agent-owned global .env so the chat session that follows picks them up via
...
// re-run of `novel-agent setup` re-pins the current value into .env (a
...
// are appended — so re-running `novel-agent setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("novel-agent") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("novel-agent setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("novel-agent chat", i18n.M.StepChatDesc)
	step(`novel-agent run "task"`, i18n.M.StepRunDesc)
...
func configAutoPlanCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./novel-agent.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  novel-agent config auto-plan [--local] [off|on]
`)
}
