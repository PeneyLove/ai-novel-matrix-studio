...
// parsing, assembly from config, and exit codes. The core is config-driven â€?// providers and tools are resolved from configuration, not hardcoded.
package cli
...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/boot"
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/provider/openai"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/serve"
	"time"
...
func Run(args []string, version string) int {
	...
	case "init":
		// Project memory (AGENTS.md) is model-generated in-session â€?`/init` runs
...
// the agent's typed event stream â€?runAgent passes a TextSink that renders to
...
// so the same typed stream the chat TUI consumes reaches web clients â€?the
...
func runServe(args []string) int {
	...
	// Auto-save target: reuse the resumed file, else a fresh one â€?same as chat.
	if *resume != "" {
	...
	fmt.Printf("reasonix serve â€?%s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string) int {
	...
	if err != nil && errors.Is(err, boot.ErrUnknownModel) && isInteractive() && config.SourcePath() == "" {
		...
		// With a config present, fall through to the descriptive error â€?re-running
	...
	// Initial terminal width â€?the TUI re-flows on every WindowSizeMsg so
	...
	// task tool) keep their headless gate from setup â€?no UI to prompt through.
	...
	// model (carrying the conversation). It must NOT touch the running model â€?	// runModelSubcommand performs the swap on the live copy. The same stable sink
	...
	// all work â€?the bubbletea-managed region is just the bottom input/status.
	p := tea.NewProgram(m)
	final, runErr := p.Run()
	...
	// subprocesses â€?operations that corrupt bubbletea's terminal raw mode
...
// displayPath shortens a home-relative path to ~/â€?for readable wizard output.
...
// and API keys to the reasonix-owned global .env â€?never a project's own .env.
// Project memory is a separate concern â€?the in-session `/init` skill generates
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("â—?) + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("reasonix")),
		"",
	...
	fmt.Printf("\n%s %s\n", green("âœ?), fmt.Sprintf(i18n.M.WroteFileFmt, displayPath(configPath)))
	...
	if len(envLines) > 0 {
		...
		fmt.Printf("%s %s\n", green("âœ?), fmt.Sprintf(i18n.M.WroteFileFmt, displayPath(envPath)))
	}

	fmt.Printf("\n%s %s\n", accent("â—?), i18n.M.SetupComplete)
	return 0
}
...
func pickSessionToResume() (string, int) {
	...
	for i, s := range sessions {
		...
		items[i] = menuItem{
			name: when,
			desc: fmt.Sprintf("%d turns Â· %s", s.Turns, preview),
		}
	}
...
func selectLanguage() (string, error) {
	...
	items := []menuItem{{name: "English"}, {name: "ä¸­æ–‡ (ç®€ä½?"}}
	tags := []string{"en", "zh"}
	if detected == "zh" {
		items[0], items[1] = items[1], items[0]
		tags[0], tags[1] = tags[1], tags[0]
	}
	idx, err := selectOne("Language Â· è¯­è¨€", items)
	if err != nil {
		return "", err
	}
	return tags[idx], nil
...
// (DeepSeek / MiMo / custom / â€? and returns one ProviderEntry per chosen
...
// model list when the call fails â€?offline first-run, missing key, or a
...
func selectEnabledProviders(providers []config.ProviderEntry) ([]config.ProviderEntry, error) {
	...
	for _, fi := range famIdxs {
		...
		// flash and pro SKUs), not just the first â€?so a failed /models probe
...
// later and skips re-asking. A blank entry is fine â€?the static fallback covers it.
...
// On any failure â€?no base URL, no key set yet (the key is collected in a
// later wizard step), network/auth error, or a vendor without /models â€?it
...
// base URL, kind, context window, pricing, and effort â€?the things that
...
// members so each model keeps its own entry â€?and therefore its own pricing,
...
// â€?the key step happens later in the wizard and .env is updated then.
...
// â€?Anthropic's own API has no public model list â€?so on any failure the
...
// families (DeepSeek, MiMo) even when the loaded config replaced them â€?a
...
// config.Load. The user can hit Enter to skip â€?the chat banner falls back to a
...
// set â€?either by loadDotEnv from .env, or by an earlier wizard step that
// called os.Setenv (the URL-fetch flow asks for the key once so it can call
// /models) â€?the existing value is reused and a single-line confirmation is
...
func configureKeys(selected []config.ProviderEntry, r io.Reader, w io.Writer) []string {
	...
	for _, p := range selected {
		...
		if cur := os.Getenv(p.APIKeyEnv); cur != "" {
			fmt.Fprintf(w, "  %s %s\n", green("âœ?), fmt.Sprintf(i18n.M.APIKeyAlreadySetFmt, p.APIKeyEnv))
...
// stdin and stdout â€?required for prompting. Redirected or piped I/O is not
...
// are appended â€?so re-running `reasonix setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	// step inline â€?first run already chose language and providers, so we don't
	...
	b.WriteString(boxed([]string{
		accent("â—?) + " " + bold("reasonix") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	for i, p := range cfg.Providers {
		...
		dot, status := yellow("â—?), dim(i18n.M.NoKey)
		if p.APIKey() != "" {
			dot, status = green("â—?), green(i18n.M.Ready)
			ready++
		}
		fmt.Fprintf(&b, "  %s %s %s%s\n", padRight(label, 8), dot, padRight(p.Name, 16), status)
	}

	fmt.Fprintf(&b, "\n  %s %s\n", accent("â–?), bold(i18n.M.GetStarted))
	n := 1
