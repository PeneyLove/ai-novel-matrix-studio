...
func renderSkillSearchBox(query string, active bool, w int) string {
	...
	var b strings.Builder
	b.WriteString(dim("  鈺? + strings.Repeat("鈹�", boxWidth-2) + "鈺?))
	b.WriteByte('\n')
	b.WriteString(dim("  鈹?" + text + " 鈹?))
	b.WriteByte('\n')
	b.WriteString(dim("  鈺? + strings.Repeat("鈹�", boxWidth-2) + "鈺?))
	b.WriteByte('\n')
	return b.String()
...
func renderSkillRow(num int, selected bool, s skill.Skill, enabled bool, w int) string {
	...
	status := "鉁?" + i18n.M.SkillPickerAvailableLabel
	if enabled {
		status = viewStatus(status)
	} else {
		status = viewMeta("鈼?" + i18n.M.SkillPickerDisabledLabel)
	}
	meta := skillRowMeta(s)
	number := fmt.Sprintf("%2d. ", num)
	line := fmt.Sprintf("%s%s%s 路 %s 路 %s", prefix, number, name, status, viewMeta(meta))
