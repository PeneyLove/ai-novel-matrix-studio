...
func (p *mcpManager) footerHint() string {
	switch p.stage {
	case mcpStageDetail:
		return "鈫?鈫?navigate 路 Enter to select 路 Esc to back"
	case mcpStageTools, mcpStageLogs:
		return "Esc to back"
	case mcpStageMode:
		return "Enter to apply 路 Esc to back"
	case mcpStageConfirmRemove, mcpStageConfirmClearAuth:
		return "Enter to select 路 y confirm 路 n cancel 路 Esc to back"
	default:
		if len(p.snapshot.servers) == 0 {
			return "鈫?鈫?navigate 路 Enter to confirm 路 Esc to cancel"
		}
		return "鈫?鈫?navigate 路 Enter for details 路 Esc to close"
	}
}
...
func (p *mcpManager) renderList(width int) string {
	...
	if start > 0 {
		fmt.Fprintf(&b, "%s\n", viewMeta(fmt.Sprintf("鈫?%d more above", start)))
	}
	lastGroup := ""
	for i := start; i < end; i++ {
	...
	if end < len(p.snapshot.servers) {
		fmt.Fprintf(&b, "%s\n", viewMeta(fmt.Sprintf("鈫?%d more below", len(p.snapshot.servers)-end)))
	}
...
func (p *mcpManager) renderListRow(i int, s mcpServerView, width int) string {
	...
	meta := fmt.Sprintf("%s 路 %s", status, countText(s.Tools, "tool"))
	if s.Prompts > 0 {
		meta += " 路 " + countText(s.Prompts, "prompt")
	}
	if s.Resources > 0 {
		meta += " 路 " + countText(s.Resources, "resource")
	}
	if s.Transport != "" {
		meta = s.Transport + " 路 " + meta
	}
	...
	return fmt.Sprintf("%s%s 路 %s", prefix, name, viewMeta(meta))
}
...
func mcpAuthLabel(v mcpServerView) string {
	switch {
	case v.Status == "connected":
		return green("鉁?authenticated")
	case mcpAuthStatus(v) == mcpdiag.AuthRequired:
		return red("鉁?not authenticated")
