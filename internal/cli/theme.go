...
// withoutTerminalProbe resolves a theme with the OSC background probe disabled 鈥?// for callers running while something else (the live TUI) owns stdin, where a
...
func describeCLIThemes() string {
	...
	fmt.Fprintf(&b, "%s  auto 路 light 路 dark\n", dim("modes:"))
	for _, st := range cliThemeStyles {
		marker := "  "
...
func (m *chatTUI) themeArgItems(val string) ([]compItem, int, bool) {
	...
	for _, it := range items {
		...
		out = append(out, compItem{label: it.label, insert: it.label, hint: it.mode + " 路 " + it.desc})
	}
	for _, st := range cliThemeStyles {
		if cur != "" && !strings.HasPrefix(st.name, cur) {
			continue
		}
		hint := st.mode + " 路 " + st.description
		if st.name == activeCLITheme.style {
