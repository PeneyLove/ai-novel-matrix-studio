...
func selectOne(label string, items []menuItem) (int, error) {
	...
	w := os.Stdout
	fmt.Fprintf(w, "%s %s  %s\r\n\r\n", accent("â–?), bold(label), dim(i18n.M.SelectOneHint))
	...
	render := func() {
		for i, it := range items {
			...
			if i == sel {
				fmt.Fprintf(w, "\r\033[K%s\r\n", reverse(fmt.Sprintf(" â?%s %s ", name, it.desc)))
			} else {
...
func selectMany(label string, items []menuItem) ([]int, error) {
	...
	w := os.Stdout
	fmt.Fprintf(w, "%s %s  %s\r\n\r\n", accent("â–?), bold(label), dim(i18n.M.SelectManyHint))
	...
	render := func() {
		for i, it := range items {
			...
			if i == cur {
				fmt.Fprintf(w, "\r\033[K%s\r\n", reverse(fmt.Sprintf(" â?%s %s %s ", box, name, it.desc)))
			} else {
