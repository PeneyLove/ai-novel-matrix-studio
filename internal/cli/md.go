...
func isCJKPunct(r rune) bool {
	...
	switch {
	case r >= 0x3000 && r <= 0x303F: // CJK Symbols and Punctuation (ã€‚ã€etc.)
		return true
...
func (r *mdRenderer) renderBlock(buf *strings.Builder, node ast.Node, src []byte, indent int) {
	...
	case *ast.ThematicBreak:
		...
		buf.WriteString(dim(strings.Repeat("â”€", w)))
		buf.WriteString("\n\n")
	default:
...
func (r *mdRenderer) renderHeading(buf *strings.Builder, n *ast.Heading, src []byte, indent int) {
	...
	if n.Level == 1 {
		buf.WriteString(strings.Repeat(" ", indent))
		buf.WriteString(accent(strings.Repeat("â”€", visibleWidth(inline))))
		buf.WriteString("\n")
	}
	buf.WriteString("\n")
}
...
func (r *mdRenderer) renderTable(buf *strings.Builder, n *extast.Table, src []byte, indent int) {
	...
	if len(header) > 0 {
		...
		for i := range widths {
			if i > 0 {
				buf.WriteString(dim("â”€â”¼â”€"))
			}
			buf.WriteString(dim(strings.Repeat("â”€", widths[i])))
		}
		buf.WriteByte('\n')
	}
	for _, row := range rows {
