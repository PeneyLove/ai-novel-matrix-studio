...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/fileref"
...
func (m *chatTUI) slashItems() []compItem {
	items := []compItem{
		...
		{label: "/rag", insert: "/rag ", hint: i18n.M.CmdRag, descend: true},
...
func (m *chatTUI) branchArgItems(val string) ([]compItem, int, bool) {
	...
	for _, b := range branches {
		...
		if hint != "" {
			hint = fmt.Sprintf("%d turns Â· %s", b.Turns, hint)
		}
