...
// session into the running controller in place 鈥?keeping the current model and
...
func (m *chatTUI) showSessions(sessions []agent.SessionInfo) {
	...
	b.WriteString(dim("  路 " + i18n.M.ResumeListHeader + "\n"))
...
func (m *chatTUI) resumeArgItems(val string) ([]compItem, int, bool) {
	...
	for i, s := range recentSessions(m.ctrl.SessionDir()) {
		...
		hint := fmt.Sprintf("%s 路 %s", s.ModTime.Local().Format("01-02 15:04"), sessionSummary(s))
		out = append(out, compItem{label: idx, insert: idx, hint: hint})
	}
	return out, from, true
}

// sessionSummary is the "N turns 路 first message" line shared by the /resume
...
func sessionSummary(s agent.SessionInfo) string {
	...
	return fmt.Sprintf("%d turns 路 %s", s.Turns, preview)
}
