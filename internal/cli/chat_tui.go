...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/agent"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/command"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/config"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/control"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/event"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/hook"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/i18n"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/memory"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/outputstyle"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/plugin"
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/tool/builtin"
)

// chatTUI is a bubbletea Model that runs a chat session in the terminal's
// normal buffer (no alt-screen). Finalized output 鈥?user bubbles, tool dispatch
// lines, usage lines, reasoning, and the rendered assistant answer 鈥?is
...
// bottom 鈥?input box, status line, an optional approval/plan banner, and the
// autocomplete menu 鈥?and it is kept a stable height (it changes only on
...
type chatTUI struct {
	...
	// events) for the live "鈫揘" readout in the running status line.
	turnTokens int

	// balance is the last-fetched wallet-balance readout (e.g. "楼110.00"), "" when
	...
	// browsing/editing via 鈫?鈫?during tuiRunning. -1 means "not browsing".
	queueEditCursor int
	// queueEditDraft saves the in-progress input text when the user first
	// presses 鈫?to browse the queue, so it can be restored when the cursor
	// moves past the end.
	...
	// finalize 鈥?at a tool/usage boundary or turn end 鈥?not previewed live, so
	...
	// reasoningLineIdx is the transcript index of the live "鈻?thinking鈥? marker
	// while a reasoning block streams; it's rewritten to "鈻?thought for Ns" when
	...
	// reasoningView is a bounded trailing window (鈮?reasoningViewMax bytes) of the
	...
	// is the call ID it belongs to. Only a bounded tail is kept 鈥?the last few
	// complete lines (toolTail) plus the in-progress one (toolPartial) 鈥?so a
	// high-output command can't balloon memory or cost O(n虏) re-splitting;
	...
	// toolStreamStart / toolStreamFrame drive the "鈳?working 路 Ns" line shown
	...
	// while the user browses saved sessions with 鈫?鈫?and confirms with Enter.
	...
	// model 鈥?the swap happens in runModelSubcommand on the running copy). nil
...
// previous controller that must be closed after the switch 鈥?its cleanup
...
// delta < 0 means 鈫?(older), delta > 0 means 鈫?(newer). Returns true if the
// input was updated.
func (m *chatTUI) navigateQueue(delta int) bool {
	...
	if cursor < 0 {
		if delta > 0 {
			return false // already at "new draft" 鈥?nothing newer
		}
		// First 鈫? save the current draft and jump to the last queued item.
...
func (m chatTUI) update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		// Commit the banner 鈥?and a resumed session's transcript 鈥?once, now
	...
	case tea.MouseClickMsg:
		...
		// left-press in the transcript region begins a text selection 鈥?unless
	...
	case tea.KeyPressMsg:
		...
		// something") mode, the keystroke goes to the textarea 鈥?Enter confirms the
		// custom answer, Esc backs out of typing 鈥?so input/IME work as usual.
		if m.chooser != nil {
			if m.chooser.typing {
				switch msg.String() {
				case "enter":
		...
		// (鈫?鈫?move, Tab/Enter accept, Esc close); everything else falls through
		...
		if m.completion.active {
			...
			case "tab", "enter":
				...
				// fall through to submit the command (/resume 3 鈫?resume session 3).
			...
			case "esc":
				...
				if m.state == tuiRunning {
					break // a turn is running 鈥?also cancel it via the main Esc handler
				}
				return m, nil
			}
		}
		...
		case "enter":
			// Don't reset queue navigation 鈥?the Enter handler below needs
		...
		case "esc":
			...
			// Shift+Tab cycle (/plan 鈫?YOLO 鈫?normal) or --yolo flag. Scrollback is
		...
		case "ctrl+c", "super+c", "meta+c":
			if m.state == tuiRunning {
				if m.bubblePending {
					m.unsendPending() // server not yet replied 鈥?restore text, leave no trace
				} else {
					m.ctrl.Cancel()
				}
				return m, nil
		...
		case "ctrl+b":
			...
			return m, finalize(m, cmds)
		case "shift+tab":
			// Cycle auto 鈫?plan 鈫?YOLO; allowed mid-turn so the user can flip the
		...
		case "enter":
			if m.state == tuiRunning {
				...
				if m.queueEditCursor >= 0 && m.queueEditCursor < len(m.pendingInterject) {
					...
					m.notice("feedback queued 鈥?will send when the current turn finishes")
					m.queueEditCursor = -1
					m.queueEditDraft = ""
				}
				m.input.Reset()
	...
	case agentEventMsg:
		...
		// batch instead of one per event 鈥?bounds the O(transcript) re-render cost
	...
	drain:
		...
		// A turn just spent tokens (and money) 鈥?refresh the balance readout and
		...
		if turnDone {
			...
			if len(m.pendingInterject) > 0 {
				...
				// Reset queue navigation 鈥?the indices shifted.
				m.queueEditCursor = -1
				m.queueEditDraft = ""
	...
	case modelSwitchMsg:
		...
		if msg.err != nil {
			...
			// Build failed 鈥?no old controller to retire.
		} else {
			m.ctrl = msg.ctrl
			m.label = msg.label
			...
			// closed here or in the build goroutine 鈥?Close() runs
			...
			// Do NOT re-issue waitForAgentEvent here 鈥?the goroutine from the
...
// wrapped answer 鈥?which the markdown renderer already fit to width 鈥?are safe;
...
func clampWidth(s string, width int) string {
	...
	// boundaries, preserving ANSI and counting wide chars 鈥?exactly what we want,
...
// from. Re-rendering the full chain of thought on every delta was O(n虏) (a 2k-
...
// "鈳? connector that ties the block to the "鈻?thinking鈥? marker above it. A
...
// oldest when it exceeds the window (the backing array stays 鈮?window+1).
...
// "鈳?N lines" summary, so the scrollback keeps a marker of the run without the
...
func (m *chatTUI) collapseToolOutput(id string) {
	...
	if n == 0 {
		...
		if total > shellPreviewLines {
			...
			preview[shellPreviewLines] = dim(fmt.Sprintf("鈥?%d more lines (click/Ctrl+B)", total-shellPreviewLines))
...
func (m *chatTUI) toggleShellOutput() {
	...
	if m.shellExpanded[lastID] {
		...
		if total > shellExpandMaxLines {
			rendered = append(rendered, dim(fmt.Sprintf("鈥?%d more lines", total-shellExpandMaxLines)))
		}
...
// "鈳?working 路 Ns" line of a tool that hasn't streamed output yet.
var toolWorkingFrames = []string{"鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?, "鉅?}

// beginToolRunning opens an empty live block under a just-dispatched tool card,
// keyed by the call id. tickToolRunning fills it with a "working 路 Ns" line each
...
// commitReasoning closes the live thinking block: the "鈻?thinking鈥? marker is
// rewritten to a dim "鈻?thought for Ns" summary and the streamed text below it is
// removed (collapsed) 鈥?kept only in verbose mode. The viewport re-wraps from
...
func (m *chatTUI) commitReasoning() {
	...
	m.transcript[m.reasoningLineIdx] = dim(fmt.Sprintf("  鈻?"+i18n.M.ChatThoughtForFmt, secs))
	if m.reasoningTextIdx >= 0 {
...
// rewritten in place as later paragraphs land 鈥?so a long reply appears chunk by
...
// commitPending freezes the full accumulated answer as markdown 鈥?overwriting the
...
// the thinking鈫抋nswer break the renderer strips.
...
// markdown blocks 鈥?text up to the last blank line outside any open fenced code
...
// (planApprovalTool), allowing also drops the local [plan] tag 鈥?the
...
func (m chatTUI) View() tea.View {
	...
	switch {
	case m.rewind != nil:
		status = "  " + modeTag + " 路 鉄?rewind"
	case m.resumePick != nil:
		status = "  " + modeTag + " 路 " + i18n.M.StatusResumePicker
	case m.mcp != nil:
		status = "  " + modeTag + " 路 MCP"
	case m.skillPick != nil:
		status = "  " + modeTag + " 路 " + i18n.M.SkillPickerStatusLabel
	case m.chooser != nil:
		status = "  " + modeTag + " 路 " + i18n.M.ChatStatusQuestion
	case m.pendingApproval != nil && m.pendingApproval.Tool == planApprovalTool:
		status = "  " + modeTag + " 路 " + i18n.M.ChatStatusPlanApproval
	case m.pendingApproval != nil:
		status = "  " + modeTag + " 路 " + i18n.M.ChatStatusToolApproval
	case shellMode:
		status = "  " + modeTag + " 路 " + i18n.M.ShellModeHint
	case m.ctrl.Bypass():
		status = "  " + modeTag + " 路 " + i18n.M.ChatStatusYoloIdle + " " + dim("("+i18n.M.ChatStatusCycleHint+")")
	default:
		status = "  " + modeTag + " 路 " + i18n.M.ChatStatusIdle + " " + dim("("+i18n.M.ChatStatusCycleHint+")")
	}
	if et := m.effortTag(); et != "" {
		status += " 路 " + et
	}
	if gt := m.gitTag(boxW - visibleWidth(status) - visibleWidth(" 路 ")); gt != "" {
		status += " 路 " + gt
	}
	// The spinning "thinking鈥? indicator is its own line ABOVE the input box (shown
	...
	if m.state == tuiRunning {
		if m.retryAttempt > 0 {
			...
			if m.turnTokens > 0 {
				working += " 路 鈫? + shortTokens(m.turnTokens)
			}
			if n := len(m.pendingInterject); n > 0 {
				if n == 1 {
					working += dim(" 路 鉁?feedback queued")
				} else {
					working += dim(fmt.Sprintf(" 路 鉁?%d queued", n))
				}
			}
		}
	}
	...
	// resize repaints cleanly 鈥?no scrollback reflow, no ghost borders.
...
func compactionCardLines(c event.Compaction) []string {
	...
	header := fmt.Sprintf("%s 路 %d %s 路 %s", i18n.M.CompactionTitle, c.Messages, i18n.M.CompactionUnit, trigger)
	lines := []string{accent("鈼?" + header)}
	for _, ln := range strings.Split(strings.TrimRight(c.Summary, "\n"), "\n") {
		lines = append(lines, dim("  鈹?"+ln))
	}
	if c.Archive != "" {
		lines = append(lines, dim("  鈹?archived "+c.Archive))
	}
	return lines
}
...
func (m chatTUI) contextTag() string {
	...
	body := fmt.Sprintf("%s ctx (%d%%) 路 %d%% to compact", shortTokens(used), pct, left)
	switch {
	case pct >= threshold:
		return themeStyle(activeCLITheme.danger).Render(fmt.Sprintf("%s ctx (%d%%) 路 compacting soon", shortTokens(used), pct))
	case left <= 10:
		return themeStyle(activeCLITheme.warn).Render(body)
	default:
		return dim(body)
...
// cacheTag renders both prompt cache-hit rates for the status line 鈥?// "turn hit 88.00% 路 avg 78.00%": the single-turn rate (latest turn, the higher/steeper
// number on a non-compacting DeepSeek session) and the session-aggregate rate
// 危hit/危(hit+miss) (the steadier, cost-oriented number that matches the legacy
...
// shortTokens prints token counts compactly: 1_500 鈫?"1.5K", 142_000 鈫?"142.0K", 1_000_000 鈫?"1.0M".
...
func (m chatTUI) renderApprovalBanner() string {
	...
	if m.pendingApproval.Tool == planApprovalTool {
		return approvalBannerStyle.Width(w).Render("鈴?" + i18n.M.PlanApprovalPrompt)
	}
	...
	return approvalBannerStyle.Width(w).Render("鈴?" + text)
}
...
func (m chatTUI) renderTodoPanel() string {
	...
	if done == len(p.Todos) {
		return "" // all finished 鈥?clear the panel
	}
	...
	for _, t := range p.Todos {
		...
		switch t.Status {
		case "completed":
			b.WriteString(indent + green("鉁?) + " " + dim(t.Content) + "\n")
		case "in_progress":
			label := t.Content
			if t.ActiveForm != "" {
				label = t.ActiveForm
			}
			b.WriteString(indent + yellow("鈻?"+label) + "\n")
		default:
			b.WriteString(indent + dim("鈼?"+t.Content) + "\n")
		}
	}
...
func truncateSubject(s string, width int) string {
	...
	return ansi.Truncate(s, max, "鈥?)
}

// clampStatusLine truncates a status line to `width` visible columns, ANSI-aware,
// appending an ellipsis and a reset. The bottom region must stay a fixed height 鈥?// the non-alt-screen renderer commits scrollback by clearing the prior frame's
...
// (ctx 路 cache 路 avg 路 jobs 路 balance) it carries on a narrow terminal.
func clampStatusLine(s string, width int) string {
	// ansi.Truncate is ANSI-aware, counts wide chars, and appends the tail when
	// it actually clips 鈥?one row regardless of how many tags the status carries.
	return ansi.Truncate(s, width, "鈥?)
}
...
func foldedPasteLabel(id, lines int) string {
	return fmt.Sprintf("[Pasted text #%d 路 %d lines]", id, lines)
}
...
// cycleMode advances the input mode normal 鈫?plan 鈫?YOLO 鈫?normal (Shift+Tab),
...
func (m *chatTUI) cycleMode() {
	switch {
	case m.ctrl.Bypass():
		m.ctrl.SetBypass(false) // YOLO 鈫?normal
	case m.planMode:
		m.planMode = false
		m.ctrl.SetPlanMode(false)
		m.ctrl.SetBypass(true) // plan 鈫?YOLO
	default:
		m.planMode = true
		m.ctrl.SetPlanMode(true) // normal 鈫?plan
	}
}
...
func (m *chatTUI) toggleVerboseReasoning(notify bool) {
	...
	if m.showReasoning {
		m.notice("verbose on 鈥?thinking text will be shown")
	} else {
		m.notice("verbose off 鈥?thinking text will stay collapsed")
	}
}
...
// just-sent text to the input box, and cancels the request 鈥?marking the turn
...
// the reasoning and answer streamed so far, then commits its own line 鈥?// preserving order. Switching on the event Kind replaces the old prefix-sniffing
...
func (m *chatTUI) ingestEvent(e event.Event) {
	...
	// The first packet of any kind means the server replied 鈥?confirm the send so
	...
	case event.Reasoning:
		if m.reasoningLineIdx < 0 {
			...
			m.commitLine(dim("  鈻?" + i18n.M.ChatThinking))
	...
	case event.Message:
		// The answer stream is complete 鈥?freeze reasoning + the markdown answer.
		m.commitReasoning()
		m.commitPending()

	case event.ToolDispatch:
		// The early (partial) dispatch only carries the name 鈥?the full dispatch
	...
	case event.ToolResult:
		...
		// call surfaces a red "鈴?Verb 鈯?<reason>" card. A live-output block (bash)
		// collapses to a one-line "鈳?N lines" summary first.
		...
		if e.Tool.Err != "" {
			m.finalizeStreamed()
			m.commitLine("  " + red("鈼?) + " " + bold(toolDisplayName(e.Tool.Name)) + " " + red("鈯?"+e.Tool.Err))
		}
	...
	case event.CompactionStarted:
		m.finalizeStreamed()
		m.commitLine(dim("  鈰?" + i18n.M.CompactionWorking))
	...
	case event.TurnDone:
		// The turn settled 鈥?freeze anything still streaming, surface a real error,
...
func (m *chatTUI) commandNames() string {
	...
	return strings.Join(names, " 路 ")
}

// runMCPSubcommand handles "/mcp" (status), "/mcp add 鈥? (connect a server live
// and persist it), and "/mcp remove <name>" (disconnect + drop from config). Add
// connects synchronously 鈥?like /compact, an explicit command may briefly block
...
func (m *chatTUI) runMCPSubcommand(input string) {
	...
	case "add":
		...
		m.notice(fmt.Sprintf("connected %s 鈥?%d tools, saved to config (available next message)", entry.Name, n))
	case "connect":
		if len(args) < 3 {
		...
		m.host = m.ctrl.Host()
		m.notice(fmt.Sprintf("connected %s 鈥?%d tools (available next message)", args[2], n))
	case "remove", "rm":
		if len(args) < 3 {
	...
	default:
		m.notice("unknown /mcp subcommand " + args[1] + " 鈥?try: /mcp, /mcp list, /mcp show, /mcp add, /mcp connect, /mcp remove")
	}
}

// runRAGSubcommand handles "/rag" (status), "/rag init <path>" (set up local
// ragCore directory), "/rag remote <endpoint>" (configure remote RAG),
// "/rag enable|disable", "/rag topk <N>", and "/rag config" (show raw config).
func (m *chatTUI) runRAGSubcommand(input string) {
	args := tokenizeArgs(input) // args[0] == "/rag"
	cfg, err := config.Load()
	if err != nil {
		m.notice("rag: cannot load config: " + err.Error())
		return
	}
	...
	case "init":
		if len(args) < 3 {
			m.notice("usage: /rag init <path> — path to ragCore/ or a directory of novel .txt files")
			return
		}
		path := args[2]
		stats, err := scanRagCore(path)
		if err != nil {
			m.notice("rag init: " + err.Error())
			return
		}
		if err := cfg.SetRAGLocalPath(path); err != nil {
			m.notice("rag init: save config: " + err.Error())
			return
		}
		m.commitLine(fmt.Sprintf("RAG knowledge base initialized — %s\n  %s",
			renderRAGStats(stats), dim(path)))
		// Reload RAG config so the tool picks up the change immediately
		builtin.SetRAGConfig(cfg.RAG)
		m.notice("RAG local mode enabled — rag_search is now active")

	case "remote":
		if len(args) < 3 {
			m.notice("usage: /rag remote <endpoint> [--key-env VAR] [--index NAME]")
			return
		}
		endpoint := args[2]
		if err := cfg.SetRAGEndpoint(endpoint); err != nil {
			m.notice("rag remote: " + err.Error())
			return
		}
		...
		for i := 3; i < len(args); i++ {
			...
			case "--key-env":
				if i+1 < len(args) {
					cfg.SetRAGAPIKeyEnv(args[i+1])
					i++
				}
			case "--index":
				if i+1 < len(args) {
					cfg.SetRAGIndexName(args[i+1])
					i++
				}
			}
		}
		m.commitLine(fmt.Sprintf("RAG remote configured — %s\n  %s", endpoint, dim(fmt.Sprintf("index=%s key_env=%s", cfg.RAG.IndexName, cfg.RAG.APIKeyEnv))))
		builtin.SetRAGConfig(cfg.RAG)
		m.notice("RAG remote mode enabled")

	case "enable":
		if err := cfg.SetRAGEnabled(true); err != nil {
			m.notice("rag enable: " + err.Error())
			return
		}
		builtin.SetRAGConfig(cfg.RAG)
		m.notice("RAG enabled")

	case "disable":
		if err := cfg.SetRAGEnabled(false); err != nil {
			m.notice("rag disable: " + err.Error())
			return
		}
		builtin.SetRAGConfig(cfg.RAG)
		m.notice("RAG disabled")

	case "topk":
		if len(args) < 3 {
			m.notice("usage: /rag topk <N> (1-50)")
			return
		}
		var k int
		if _, err := fmt.Sscanf(args[2], "%d", &k); err != nil || k < 1 || k > 50 {
			m.notice("rag topk: N must be between 1 and 50")
			return
		}
		if err := cfg.SetRAGTopK(k); err != nil {
			m.notice("rag topk: " + err.Error())
			return
		}
		builtin.SetRAGConfig(cfg.RAG)
		m.notice(fmt.Sprintf("RAG top_k set to %d", k))

	case "config":
		m.showRAGConfigRaw(cfg)

	default:
		m.notice("unknown /rag subcommand " + args[1] + " — try: /rag, /rag init, /rag remote, /rag enable, /rag disable, /rag topk, /rag config")
	}
}

// showRAGStatus prints the current RAG configuration in a readable format.
func (m *chatTUI) showRAGStatus(cfg *config.Config) {
	var b strings.Builder
	b.WriteString(viewHeader("RAG knowledge base"))
	b.WriteString("\n")

	if cfg.RAG.Enabled {
		b.WriteString("  status:  " + accent("enabled") + "\n")
	} else {
		b.WriteString("  status:  " + dim("disabled") + "\n")
	}
	...
	if cfg.RAG.Mode == "local" || cfg.RAG.LocalPath != "" {
		b.WriteString("  path:    " + dim(cfg.RAG.LocalPath) + "\n")
		if cfg.RAG.Mode == "local" && cfg.RAG.LocalPath != "" {
			if stats, err := scanRagCore(cfg.RAG.LocalPath); err == nil {
				b.WriteString("  " + renderRAGStats(stats) + "\n")
			}
		}
	} else {
		b.WriteString("  endpoint:" + dim(cfg.RAG.Endpoint) + "\n")
		b.WriteString("  index:   " + dim(cfg.RAG.IndexName) + "\n")
		b.WriteString("  key_env: " + dim(cfg.RAG.APIKeyEnv) + "\n")
	}

	if cfg.RAG.TopK > 0 {
		b.WriteString(fmt.Sprintf("  top_k:   %d\n", cfg.RAG.TopK))
	}

	m.commitLine(strings.TrimRight(b.String(), "\n"))
}

// showRAGConfigRaw dumps the raw [rag] section as text.
func (m *chatTUI) showRAGConfigRaw(cfg *config.Config) {
	var b strings.Builder
	b.WriteString(viewHeader("RAG raw config"))
	b.WriteString("\n")
	fmt.Fprintf(&b, "  enabled     = %v\n", cfg.RAG.Enabled)
	fmt.Fprintf(&b, "  mode        = %q\n", cfg.RAG.Mode)
	fmt.Fprintf(&b, "  endpoint    = %q\n", cfg.RAG.Endpoint)
	fmt.Fprintf(&b, "  api_key_env = %q\n", cfg.RAG.APIKeyEnv)
	fmt.Fprintf(&b, "  index_name  = %q\n", cfg.RAG.IndexName)
	fmt.Fprintf(&b, "  top_k       = %d\n", cfg.RAG.TopK)
	fmt.Fprintf(&b, "  local_path  = %q\n", cfg.RAG.LocalPath)
	m.commitLine(strings.TrimRight(b.String(), "\n"))
}

// ragCoreStats holds scan results.
type ragCoreStats struct {
	Genres    int
	Novels    int
	Volumes   int
	Chapters  int
...
// genreDirNames lists the 6 recognised genre directory names.
var genreDirNames = map[string]bool{
	"xuanhuan": true, "dushi": true, "guyan": true,
	"xuanyi": true, "kehuan": true, "tianchong": true,
}

// scanRagCore walks a ragCore directory and counts the structure.
func scanRagCore(root string) (ragCoreStats, error) {
	var st ragCoreStats
	info, err := os.Stat(root)
	if err != nil || !info.IsDir() {
		return st, fmt.Errorf("%q is not a valid directory — expected a ragCore/ root", root)
	}

	genres, err := os.ReadDir(root)
	if err != nil {
		return st, fmt.Errorf("cannot read %q: %w", root, err)
	}

	for _, genreEntry := range genres {
		if !genreEntry.IsDir() || !genreDirNames[genreEntry.Name()] {
			continue
		}
		st.Genres++
		genrePath := filepath.Join(root, genreEntry.Name())

		novels, _ := os.ReadDir(genrePath)
		for _, novelEntry := range novels {
			if !novelEntry.IsDir() {
				continue
			}
			st.Novels++
			novelPath := filepath.Join(genrePath, novelEntry.Name())

			// Check for volume sub-dirs vs direct chapter files
			contents, _ := os.ReadDir(novelPath)
			hasVolumes := false
			for _, c := range contents {
				if c.IsDir() {
					hasVolumes = true
					st.Volumes++
					volPath := filepath.Join(novelPath, c.Name())
					chaps, _ := os.ReadDir(volPath)
					for _, ch := range chaps {
						if !ch.IsDir() && chapterFileRe.MatchString(ch.Name()) {
							st.Chapters++
						}
					}
				} else if chapterFileRe.MatchString(c.Name()) {
					st.Chapters++
				}
			}
			if !hasVolumes {
				// Count chapters directly in the title dir
				for _, c := range contents {
					if !c.IsDir() && chapterFileRe.MatchString(c.Name()) {
	...
	if st.Genres == 0 && st.Novels == 0 {
		return st, fmt.Errorf("%q does not contain any recognised genre directories (%s). Expected layout: ragCore/{genre}/{rank}_{title}/{volume}/chapter{N}.txt", root, genreListStr())
	}
...
func genreListStr() string {
	return "xuanhuan, dushi, guyan, xuanyi, kehuan, tianchong"
}

func renderRAGStats(st ragCoreStats) string {
	var parts []string
	if st.Genres > 0 {
		parts = append(parts, fmt.Sprintf("%d genres", st.Genres))
	}
	parts = append(parts, fmt.Sprintf("%d novels", st.Novels))
	if st.Volumes > 0 {
		parts = append(parts, fmt.Sprintf("%d volumes", st.Volumes))
	}
	parts = append(parts, fmt.Sprintf("%d chapters", st.Chapters))
	return strings.Join(parts, " · ")
}

// showMCPStatus queues the connected MCP servers, their counts, and the prompt
// commands / resource refs they expose 鈥?the discovery surface for /mcp.
...
// and assistant markdown. Tool messages are dropped 鈥?needed in session state
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	b.WriteString(accent("鈼?) + " " + bold("reasonix chat") + "  " + dim("路 "+label) + "\n")
