...
type chatTUI struct {
	...
	// commands are custom slash commands loaded from .novel-agent/commands; each renders
...
func (m *chatTUI) runRAGSubcommand(input string) {
	...
	case "init":
		...
		m.notice("RAG local mode enabled")
	...
	case "remote":
		...
		m.commitLine(fmt.Sprintf("RAG remote configured — %s\n  %s",
			endpoint, dim(fmt.Sprintf("index=%s key_env=%s", cfg.RAG.IndexName, cfg.RAG.APIKeyEnv))))
		builtin.SetRAGConfig(cfg.RAG)
...
func scanRagCore(root string) (ragCoreStats, error) {
	...
	if err != nil || !info.IsDir() {
		return st, fmt.Errorf("%q is not a valid directory", root)
	}
	...
	if st.Genres == 0 && st.Novels == 0 {
		return st, fmt.Errorf("%q does not contain any recognised genre directories (xuanhuan, dushi, guyan, xuanyi, kehuan, tianchong)", root)
	}
	return st, nil
}
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	b.WriteString(accent("◆") + " " + bold("novel-agent chat") + "  " + dim("· "+label) + "\n")
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.novel-agent/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)
