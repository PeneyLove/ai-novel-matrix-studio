...
// TestMathStreamingEndToEnd drives the real streaming path (ingestEvent â†?// streamAnswer/flushableMarkdownPrefix â†?renderer.Render â†?commitPending) with
...
func TestMathStreamingEndToEnd(t *testing.T) {
	...
	for _, want := range []string{"e^(iÏ€) + 1 = 0", "âˆ?, "1/(nÂ²)", "(Ï€Â²)/6", "That is all."} {
...
func TestMathStreamingStyledPath(t *testing.T) {
	...
	if stripped := ansi.Strip(out); !strings.Contains(stripped, "aÂ² + bÂ² = cÂ²") {
