...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/agent/testutil"
...
func TestStatuslinePutsGitIdentityOnModeRow(t *testing.T) {
	...
	if !strings.Contains(lines[0], "effort auto · novel-agent@codex/demo (+3 -1 ?2)") {
		t.Fatalf("mode row should include effort before git identity:\n%s", strings.Join(lines, "\n"))
	}
	if strings.Contains(lines[1], "novel-agent@codex/demo") {
