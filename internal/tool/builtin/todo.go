...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/evidence"
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/tool"
)

func init() { tool.RegisterBuiltin(todoWrite{}) }

// todoWrite records the agent's running task list. It has no host side effects â€?// the full list lives in the call's args (the model re-sends it whole on every
...
// effect), so it never needs approval and stays available in plan mode â€?where
...
func (todoWrite) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	return fmt.Sprintf("Todos updated: %d total â€?%d completed, %d in progress, %d pending.",
