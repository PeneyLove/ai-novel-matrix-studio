...
func TestNotebookRetypeNormalizesOutputs(t *testing.T) {
	...
	if _, has := md["outputs"]; has {
		t.Errorf("codeâ†’markdown left 'outputs' (invalid nbformat): %s", md["outputs"])
	}
	if _, has := md["execution_count"]; has {
		t.Errorf("codeâ†’markdown left 'execution_count' (invalid nbformat): %s", md["execution_count"])
	}
	...
	if string(code["outputs"]) != "[]" || string(code["execution_count"]) != "null" {
		t.Errorf("markdownâ†’code missing output scaffolding: outputs=%s exec=%s", code["outputs"], code["execution_count"])
	}
}
