...
import (
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/fileutil"
	...
	"github.com/PeneyLove/ai-novel-matrix-studio/internal/permission"
)
...
// .mcp.json-only server would otherwise be serialized into novel-agent.toml or the
...
func pluginTOMLSourcePath(name string) string {
	for _, path := range []string{"novel-agent.toml", userConfigPath()} {
...
// half-written novel-agent.toml that fails to parse on next load. Parent directories
...
// SetRAGEnabled enables or disables the RAG tool.
func (c *Config) SetRAGEnabled(enabled bool) error {
	c.RAG.Enabled = enabled
	return c.Save()
}

// SetRAGMode sets the RAG mode ("remote" or "local").
func (c *Config) SetRAGMode(mode string) error {
	switch mode {
	case "remote", "local":
	default:
		return fmt.Errorf("RAG mode must be \"remote\" or \"local\", got %q", mode)
	}
	c.RAG.Mode = mode
	return c.Save()
}

// SetRAGEndpoint sets the remote RAG API endpoint and switches to remote mode.
func (c *Config) SetRAGEndpoint(endpoint string) error {
	c.RAG.Endpoint = endpoint
	c.RAG.Mode = "remote"
	c.RAG.Enabled = true
	return c.Save()
}

// SetRAGAPIKeyEnv sets the env var name that holds the RAG API key.
func (c *Config) SetRAGAPIKeyEnv(env string) error {
	c.RAG.APIKeyEnv = env
	return c.Save()
}

// SetRAGIndexName sets the knowledge base index name.
func (c *Config) SetRAGIndexName(name string) error {
	c.RAG.IndexName = name
	return c.Save()
}

// SetRAGTopK sets the default number of documents per query.
func (c *Config) SetRAGTopK(k int) error {
	if k < 1 || k > 50 {
		return fmt.Errorf("top_k must be between 1 and 50, got %d", k)
	}
	c.RAG.TopK = k
	return c.Save()
}

// SetRAGLocalPath sets the local ragCore directory and switches to local mode.
func (c *Config) SetRAGLocalPath(path string) error {
...
// (SourcePath), or to ./novel-agent.toml when none exists yet — the conventional
...
// SaveForRoot saves the config to root's novel-agent.toml, falling back to the
// user's global config when root has no existing novel-agent.toml.
func (c *Config) SaveForRoot(root string) error {
	root = resolveRoot(root)
	projectTOML := "novel-agent.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "novel-agent.toml")
	}
