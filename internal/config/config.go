...
type RAGConfig struct {
	Endpoint  string `toml:"endpoint"`    // RAG service API endpoint URL (remote mode)
	APIKeyEnv string `toml:"api_key_env"` // env var holding the API key (remote mode)
	IndexName string `toml:"index_name"`  // knowledge base / index name to query (remote mode)
	TopK      int    `toml:"top_k"`       // number of documents to retrieve per query (default 5)
	Enabled   bool   `toml:"enabled"`     // whether the RAG tool is active
	Mode      string `toml:"mode"`        // "remote" or "local"
...
// background, eager); when unset it preserves the historical warmâ†’eager /
// coldâ†’background startup.
...
func Default() *Config {
	return &Config{
		...
		Providers: []ProviderEntry{
			{Name: "deepseek-flash", Kind: "openai", BaseURL: "https://api.deepseek.com", Model: "deepseek-v4-flash", APIKeyEnv: "DEEPSEEK_API_KEY", BalanceURL: "https://api.deepseek.com/user/balance", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.02, Input: 1, Output: 2, Currency: "Â¥"}},
			{Name: "deepseek-pro", Kind: "openai", BaseURL: "https://api.deepseek.com", Model: "deepseek-v4-pro", APIKeyEnv: "DEEPSEEK_API_KEY", BalanceURL: "https://api.deepseek.com/user/balance", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.025, Input: 3, Output: 6, Currency: "Â¥"}},
			{Name: "mimo-pro", Kind: "openai", BaseURL: "https://token-plan-cn.xiaomimimo.com/v1", Model: "mimo-v2.5-pro", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.025, Input: 3, Output: 6, Currency: "Â¥"}, NoProxy: true},
			{Name: "mimo-flash", Kind: "openai", BaseURL: "https://token-plan-cn.xiaomimimo.com/v1", Model: "mimo-v2.5", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.02, Input: 1, Output: 2, Currency: "Â¥"}, NoProxy: true},
		},
	}
}
