...
func main() {
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(), "e2ebench 鈥?Reasonix end-to-end benchmark.\n\n")
...
func runTask(bin, model string, t task) result {
	...
	if runErr != nil {
		...
		// still grade 鈥?a non-zero exit may just be a max-steps notice
	}
...
func render(results []result) string {
	...
	fmt.Fprintf(&b, "## 馃 Reasonix e2e benchmark\n\n")
	fmt.Fprintf(&b, "**Accuracy:** %d/%d (%s) 路 **Cache hit:** %s 路 **Tokens:** %s (prompt %s / completion %s) 路 **Compactions:** %d 路 **Cost:** %s%.4f\n\n",
	...
	for _, r := range results {
		switch {
		case r.Skipped:
			fmt.Fprintf(&b, "| `%s` | 鈴笍 skipped | 鈥?| 鈥?| 鈥?| 鈥?| 鈥?| 鈥?|\n", r.ID)
		default:
			res := "鉂?fail"
			if r.Passed {
				res = "鉁?pass"
