...
func renderDiff(r diffReport) string {
	...
	if r.sourceTouched > 0 {
		fmt.Fprintf(&b, "\nâš ï¸ The agent modified %d non-test source file(s); a green run may not reflect the PR's code. Review the diff.\n", r.sourceTouched)
	}
...
func mutationCell(r diffReport) string {
	...
	if len(r.mut.survivors) > 0 {
		cell += fmt.Sprintf(" Â· survived: `%s`", strings.Join(r.mut.survivors, "`, `"))
	}
	return cell
}
