...
> 多 Agent 会话隔离 × 三级写作缓存 × 46 个内置 Skill × 配置驱动质检引擎 — 终端里的 AI 网文创作助手
>
...
| Skill | 单一通用提示词 | **46 个内置 Skill**（编译进二进制，npm 用户直接可用） |
| 长篇一致性 | 全靠模型记忆 | **配置驱动上下文注入**（每章只注入结构化摘要，不爆上下文） |
...
| 分支 | 版本 | 内容 |
|------|------|------|
| **master** | 2.0 | 多 Agent + 46 Skill + 配置驱动质检 |
